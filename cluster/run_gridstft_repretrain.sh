#!/usr/bin/env bash
# GRID re-pretrain pipeline (M8): regenerate corpus+eval in --repr grid_stft (modulation-encoding
...
LOG=cluster/logs/m9_gridstft_repretrain.log
PY=.venv/bin/python
...
RECIPE="--steps 12000 --eval-every 2000 --eval-task modulation --n-layers 12 --router-epochs 15 --mask-percent 0.7 --w-mlm 1.0 --w-cont 0.3 --temperature 0.2 --lr 5e-4 --min-lr 1e-8 --warmup-frac 0.1 --weight-decay 0.05 --seed 42 --weights-suffix gridstft"
done3(){ d="spectro/outputs/pretrained_models/spectro_$1_p$2_gridstft_weights"; [ "$(ls "$d" 2>/dev/null|grep -c expert.pth)" = 3 ] && [ -f "$d/router.pth" ]; }
{
  echo "=== GRIDSTFT RE-PRETRAIN PIPELINE START $(date) ==="
  ...
  for P in 4 6 8; do
    for arm in transformer_synth mamba random_init raw; do
      ...
      $PY spectro/scripts/spectro_train_heads.py --arm "$arm" --patch "$P" --pool meanstd_t \
        --synth-dir "$EVAL" --weights-suffix gridstft --seed 42 || echo "$arm p$P SWEEP FAILED"
    done
  done
  $PY spectro/scripts/spectro_plot_vs_patch.py --suffix _heldout_gridstft || echo "PLOT FAILED"
  echo "=== GRIDSTFT RE-PRETRAIN PIPELINE DONE $(date) ==="
} >> "$LOG" 2>&1
