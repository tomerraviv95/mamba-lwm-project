...
# For raw mode, store scores from first patch size to reuse for others
raw_mode_scores = None
...
for patch_size in PATCH_SIZES:
    ...
    # For raw mode, only compute scores for first patch size, then reuse
    if MODEL_TYPE == "raw" and raw_mode_scores is not None:
        print(f"RAW MODE: Reusing scores from patch_4x4 for {patch_label}")
        scores = raw_mode_scores.copy()
    ...
    for t in range(1, num_tasks + 1):
        ...
        wrapper, train_losses, val_losses, test_losses, score, ground_truth, predictions = finetune(
            ...
            max_head_pars=5e6,  # Increased from config limit for raw mode
    ...
    # For raw mode, store scores from first patch size to reuse for others
    if MODEL_TYPE == "raw" and raw_mode_scores is None:
        raw_mode_scores = scores.copy()
