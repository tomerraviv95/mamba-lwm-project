...
PRETRAINED_CHECKPOINT_PATHS = {
    ...
    "wimae": "WirelessContrastiveMaskedLearning/runs/wimae_training_demo/last_checkpoint.pt",
    "contrawimae": "WirelessContrastiveMaskedLearning/runs/contrawimae_training_demo/last_checkpoint.pt",
}
...
for sample_pct in SAMPLE_PERCENTAGES:
    ...
    for t in range(1, num_tasks + 1):
        ...
        save_dir = f"submission_sample_variation_{MODEL_TYPE}/{pct_label}/task_{t}"
    ...
    pct_composite_path = f"submission_sample_variation_{MODEL_TYPE}/{pct_label}/composite_score.json"
...
aggregated_path = f"submission_sample_variation_{MODEL_TYPE}/aggregated_results.json"
...
# Create zip archive
shutil.make_archive(f"submission_sample_variation_{MODEL_TYPE}", format="zip", root_dir=f"submission_sample_variation_{MODEL_TYPE}")
...
plt.tight_layout()
plt.savefig(f"submission_sample_variation_{MODEL_TYPE}/radar_chart.png", dpi=300, bbox_inches='tight')
plt.show()