...
MODEL_TYPE = "mamba"  # Options: "transformer", "mamba", "raw"
...
def finetune(
    ...
    if input_type not in ["cls_emb", "mean_pooled", "channel_emb", "combined", "arbitrary_meanPooled", "raw"]:
    ...
    if input_type in ["cls_emb", "mean_pooled", "arbitrary_meanPooled"]:
        ...
        if sequence_length is None:
            raise ValueError("sequence_length must be provided for input_type 'raw'.")
        n_patches = sequence_length
    ...
    # For "raw" input, use patch_dim (element_length) instead of d_model
    if input_type == "raw":
